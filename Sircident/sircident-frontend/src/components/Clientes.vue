<template>
  <div class="content">
    <div
      class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100"
    >
      <v-overlay :value="loader" :z-index="'99999999'">
        <v-progress-circular
          indeterminate
          size="80"
          color="#E3F2FD"
        ></v-progress-circular>
      </v-overlay>
      <v-card>
        <v-card-title>
          Listado de los clientes
          <div class="flex-grow-1"></div>
          <v-text-field
            v-model="searchClientes"
            append-icon="search"
            label="Buscar"
            single-line
            hide-details
          ></v-text-field>
        </v-card-title>
        <v-data-table
          :headers="hTBClientes"
          :items="arrayClientes"
          :footer-props="{
            'items-per-page-options': [5, 10, 15, 20, 30],
            'items-per-page-text': 'Registros por Página',
          }"
          :items-per-page="5"
          :search="searchClientes"
          multi-sort
          class="elevation-1"
        >
          <!-- Template para form modal para agregar o actualizar categorias -->
          <template v-slot:top>
            <v-toolbar flat color="white">
              <div class="flex-grow-1"></div>
              <v-dialog v-model="modalCliente" persistent max-width="700px">
                <template v-slot:activator="{ on }">
                  <v-btn
                    elevation="10"
                    color="black"
                    dark
                    class="mb-2"
                    v-on="on"
                  >
                    Agregar&nbsp;
                    <v-icon>library_add</v-icon>
                  </v-btn>
                </template>
                <v-card>
                  <v-card-title class="headline blue lighten-5" primary-titles>
                    <span class="headline" v-text="formTitle"></span>
                  </v-card-title>
                  <v-card-text>
                    <v-container>
                      <v-form
                        ref="formCliente"
                        v-model="validForm"
                        :lazy-validation="true"
                      >
                        <v-text-field
                          append-icon="drive_file_rename_outline"

                          v-model="cliente.nombre"
                          @keyup="errorsNombre = []"
                          :rules="[(v) => !!v || 'Nombres y Apellidos Son Requeridos']"
                          label="Nombres y Apellidos"
                          required
                          :error-messages="errorsNombre"
                        ></v-text-field>
                         <v-text-field
                          append-icon="featured_play_list"
                          v-model="cliente.correo"
                          @keyup="errorsNombre = []"
                          
                          label="DUI"
                          required
                          :error-messages="errorsNombre"
                        ></v-text-field>
                         <v-text-field
                          append-icon="call"
                          v-model="cliente.telefono"
                          @keyup="errorsNombre = []"
                          :rules="[(v) => !!v || 'Télefono Es Requerido']"
                          label="Télefono"
                          required
                          :error-messages="errorsNombre"
                        ></v-text-field>
                         <v-text-field
                          append-icon="library_books"
                          v-model="cliente.direccion"
                          @keyup="errorsNombre = []"
                          :rules="[(v) => !!v || 'Expediente Es Requerido']"
                          label="Expediente"
                          required
                          :error-messages="errorsNombre"
                        ></v-text-field>
                      </v-form>
                    </v-container>
                  </v-card-text>
                  <v-divider></v-divider>
                  <v-card-actions>
                    <div class="flex-grow-1"></div>
                    <v-btn color="red darken-1" text @click="cerrarModal"
                      >Cerrar</v-btn
                    >
                    <v-btn
                      color="info darken-1"
                      :disabled="!validForm"
                      @click="saveCliente()"
                      text
                      v-text="btnTitle"
                    ></v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </v-toolbar>
          </template>

          <!-- Template que va en la tabla en la columna de Acciones (Editar,Desactivar) -->
          <template v-slot:item.action="{ item }">
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  color="#64DD17"
                  elevation="8"
                  small
                  dark
                  :disabled="item.id < 0"
                  v-on="on"
                  @click="showModalEditar(item)"
                >
                  <v-icon>create</v-icon>
                </v-btn>
              </template>
              <span>Actualizar Datos</span>
            </v-tooltip>
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  color="#FF0000"
                  class="mx-1"
                  elevation="8"
                  small
                  dark
                  :disabled="item.id < 0"
                  v-on="on"
                  @click="deleteCliente(item)"
                >
                  <v-icon>delete</v-icon>
                </v-btn>
              </template>
              <span>Eliminar Registro</span>
            </v-tooltip>
          </template>
        </v-data-table>
        <v-snackbar v-model="snackbar">
          {{ msjSnackBar }}
          <v-btn color="red" text @click="snackbar = false">Cerrar</v-btn>
        </v-snackbar>
      </v-card>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      arrayClientes: [],
      hTBClientes: [
        { text: "Nombres y Apellidos", value: "nombre" },
        { text: "DUI", value: "correo" },
        { text: "Télefono", value: "telefono" },
        { text: "Expediente", value: "direccion" },
        { text: "Acciones", value: "action", sortable: false, align: "center" },
      ],
      loader: false,
      searchClientes: "",
      modalCliente: false,
      cliente: {
        id: null,
        nombre: "",
        correo: "",
        telefono: "",
        direccion:"",
      },
      validForm: true,
      snackbar: false,
      msjSnackBar: "",
      errorsNombre: [],
      editedCliente: -1,
    };
  },
  computed: {
    formTitle() {
      return this.cliente.id == null
        ? "Agregar Cliente"
        : "Actualizar Cliente";
    },
    btnTitle() {
      return this.cliente.id == null ? "Guardar" : "Actualizar";
    },
  },
  methods: {
    fetchClientes() {
      let me = this;
      me.loader = true;
      me.$http
        .get(`${me.$url}/clientes/all`)
        .then(function(response) {
          me.arrayClientes = response.data;
          me.loader = false;
        })
        .catch(function(error) {
          me.loader = false;
          console.log(error);
        });
    },
    cerrarModal() {
      let me = this;
      me.modalCliente = false;
      setTimeout(() => {
        me.cliente = {
          id: null,
          nombre: "",
          correo: "",
          telefono: "",
          direccion:"",
        };
        me.resetValidation();
      }, 300);
    },
    resetValidation() {
      let me = this;
      me.errorsNombre = [];
      me.$refs.formCliente.resetValidation();
    },
    showModalEditar(cli) {
      let me = this;
      me.editedCliente = me.arrayClientes.indexOf(cli);
      me.cliente = Object.assign({}, cli);
      me.modalCliente = true;
    },
    saveCliente(){
      let me = this;
      if (me.$refs.formCliente.validate()) {
        let accion = me.cliente.id == null ? "add" : "upd";
        me.loader = true;
        me.$http
          .post(`${me.$url}/clientes/save`, me.cliente)
          .then(function(response) {
            me.verificarAccionDato(response.data, response.status, accion);
            me.cerrarModal();
          })
          .catch(function(error) {
            console.log(error);
            //409 Conflicts Error (Proveedor Ya Existente En la BD)
            if (error.response.status == 409) {
              me.setMessageToSnackBar("Cliente existente", true);
              me.errorsNombre = ["Nombre De Cliente Existente"];
            } else {
              me.$swal("Error", "Ocurrio Un Error Intente Nuevamente", "error");
            }
            me.loader = false;
          });
      }
    },
    /*setMessageToSnackBar(msj, estado) {
      let me = this;
      me.snackbar = estado;
      me.msjSnackBar = msj;
    },*/
    deleteCliente(cliente) {
      let me = this;
      me.editedCliente = me.arrayClientes.indexOf(cliente);
       const Toast = me.$swal.mixin({
        toast: true,
        position: "bottom-end",
        showConfirmButton: true,
        timer: 3000
      });
      me.$swal({
        title: "Eliminar Cliente?",
        text: "Una vez realizada la acción no se podra revertir ",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Si",
        cancelButtonText: "No",
        reverseButtons: true,
        focusConfirm: false,
        focusCancel: true,
        showCloseButton: true
      }).then(result => {
        if (result.value) {
          me.loader = true;
          me.$http
            .post(`${me.$url}/clientes/delete`, cliente)
            .then(function(response) {
              me.verificarAccionDato(response.data, response.status, "del");
              me.loader = false;
            })
            .catch(function(error) {
              if(error.response.status  == 510){
                Toast.fire({
                icon: "error",
                title: "No se puede eliminar este cliente"
              });
              } 
              me.loader = false;
            });
        }
      });
    },
    verificarAccionDato(cliente, statusCode, accion) {
      let me = this;

      switch (accion) {
        case "add":
          //Agrego al array de clientes el objecto que devuelve el Backend
          me.arrayClientes.unshift(cliente);
          me.loader = false;
          break;
          
        case "upd":
          //Actualizo al array de clientes el objecto que devuelve el Backend ya con los datos actualizados
          Object.assign(me.arrayClientes[me.editedCliente], cliente);
          me.loader = false;
          break;
        case "del":
          if (statusCode == 200) {
            try {
              //Se elimina del array de Categorias Activos si todo esta bien en el backend
              me.arrayClientes.splice(me.editedCliente, 1);
              //Se Lanza mensaje Final
              me.$swal.fire("Eliminado", "Cliente Eliminado", "success");
            } catch (error) {
              console.log(error);
            }
          } 
          break;
      }
    }
  },
  mounted() {
    let me = this;
    me.fetchClientes();
  },
};
</script>